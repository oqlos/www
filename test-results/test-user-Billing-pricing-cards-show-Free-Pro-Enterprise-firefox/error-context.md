...
- Name: test-user.spec.js >> Billing >> pricing cards show Free, Pro, Enterprise
- Location: e2e/test-user.spec.js:484:3
...
Locator: locator('.price-card.featured').filter({ hasText: 'OqlOS Pro' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.price-card.featured').filter({ hasText: 'OqlOS Pro' })
